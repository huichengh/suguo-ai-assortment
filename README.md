...
**本项目是一个参加比赛用的软件原型（Prototype），不是已上线的商业产品。**

| 项目 | 说明 |
|---|---|
| **性质** | 软件原型（Prototype）／参赛作品 |
| **用途** | 参加比赛，展示 AI 辅助选品决策的技术构想与工程实现 |
| **完成度** | 功能闭环可演示；算法为真实实现，非伪代码 |
| **部署状态** | ❌ 未接入任何企业生产系统　❌ 未做商业部署　❌ 无真实用户 |
| **数据来源** | 全部为**基于公开行业数据构造的模拟数据**，非任何企业真实经营数据 |
| **命名说明** | 平台名称中的「苏果」仅用于设定具体演示场景，**不代表与华润苏果存在任何合作关系、授权或背书** |

### 这个原型"真"在哪里

为了避免被误认为是空壳演示，这里明确列出**已经真实实现**的部分：

- ✅ **算法是真实实现的**，不是预设结果：品类健康度四维加权评分、Apriori 购物篮挖掘、趋势分析、综合比较评分，全部为可运行代码，输入变化则输出变化
- ✅ **数据是真实计算的**：首页 8 个 KPI 全部由算法实时算出，无硬编码数字
- ✅ **AI 助手接的是真实数据**：14 个后台工具操作真实数据集，且数据不足时会明确拒答，不编造结果
- ✅ **可完整追溯**：所有算法结果均标注来源数据集、算法名称、参数与生成时间
- ✅ **含完整测试**：五层自动化验证体系，共 139 项断言全部通过

### 这个原型"不"包含什么

- ❌ 未对接企业真实 ERP / POS / 会员系统
- ❌ 未做高并发、权限审计、数据加密等生产级加固
- ❌ AI 助手为**规则引擎 + 真实数据装配**，非接入大模型（刻意设计为离线可用，确保不编造数字）
- ❌ 部分模块（千店千面、自有品牌、新品评估）仅提供品类级结论与数据接入入口，**明确不伪造量化结果**
